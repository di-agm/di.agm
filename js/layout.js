...
// --- Helper functions for Unit Conversion (Added for functionality) ---
function mmToPx(mm) {
...
function duplicatePage(index) {
  ...
  clonedElements.forEach(element => {
    ...
    makeRotatable(element); // Re-add rotation handler
    // Re-select handler
...
function addShapeElement(type) {
  ...
  // FIXED: Removed element.style.resize = 'both'; to prevent the default box and resize handle.
...
function selectElement(element) {
  ...
  document.querySelectorAll('.text-element, .shape-element').forEach(el => {
    ...
    // FIXED: REMOVE THE VISIBLE BORDER/SHADOW FOR SHAPES on deselect
  ...
  // FIXED: APPLY A SUBTLE GLOW/SHADOW TO THE SELECTED SHAPE INSTEAD OF A THICK BORDER
